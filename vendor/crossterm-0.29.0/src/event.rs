...
pub enum Event {
    ...
    Mouse(MouseEvent),
    /// A legacy UTF-8 paste representation. Retained for compatibility with applications that
    /// construct or deserialize this variant.
    #[cfg(feature = "bracketed-paste")]
    Paste(String),
    /// Raw bytes that were pasted into the terminal. Only emitted if bracketed paste has been
...
impl Event {
    ...
    /// Returns the pasted string if this is the legacy string paste variant, otherwise `None`.
    ///
    ...
    /// Returns the pasted bytes if the event is a raw paste event, otherwise `None`.
    ///
    /// Unix bracketed-paste input is emitted through this lossless representation.
    #[cfg(feature = "bracketed-paste")]
    #[inline]
