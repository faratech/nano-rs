...
mod tests {
    ...
    fn test_parse_event_subsequent_calls() {
        ...
        assert_eq!(
            ...
            Some(InternalEvent::Event(Event::PasteBytes(
                b"on and on and on".to_vec()
            ))),
        );
    ...
    fn test_parse_csi_bracketed_paste() {
        ...
        assert_eq!(
            parse_event(b"\x1B[200~\xff\xfe\xf0\x28\x8c\xbc\x1B[201~", false).unwrap(),
            Some(InternalEvent::Event(Event::PasteBytes(vec![
                0xff, 0xfe, 0xf0, 0x28, 0x8c, 0xbc
            ]))),
            "Malformed UTF-8 in a bracketed paste is preserved byte-for-byte"
        );
    }
